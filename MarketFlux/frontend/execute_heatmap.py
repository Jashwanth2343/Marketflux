...
FILE_PATH = "src/components/MarketHeatmap.js"

with open(FILE_PATH, "r", encoding="utf-8") as f:
    content = f.read()

new_content = """import { memo, useState, useEffect } from 'react';
import { Link } from 'react-router-dom';

function getBlockSize(percent) {
    const abs = Math.abs(percent || 0);
    if (abs >= 4) return 'col-span-2 row-span-2';
    if (abs >= 2) return 'col-span-2 row-span-1';
...
function HeatmapBlock({ stock }) {
    ...
    return (
        <Link
            to={`/stock/${symbol}`}
            className={`${getBlockSize(change_percent)} ${getBlockColor(change_percent)} p-1.5 flex flex-col items-center justify-center hover:scale-[1.02] hover:z-10 hover:shadow-lg transition-all duration-200 rounded shadow-sm overflow-hidden border border-background/20`}
        >
        ...
        if (displayTab === 'ALL') {
            return Object.entries(heatmapData).map(([sector, stocks]) => {
                ...
                return (
                    <div key={sector} className="flex flex-col dark:bg-card/30 bg-white rounded-lg p-2 dark:border-border/30 border-border shadow-sm mb-3">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 auto-rows-[60px]">
                            {topStocks.map(stock => (
            ...
            return (
                <div className="flex flex-col dark:bg-card/30 bg-white rounded-lg p-2 dark:border-border/30 border-border shadow-sm h-full">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 xl:grid-cols-6 gap-1 auto-rows-[60px]">
                        {stocks.map(stock => (
                            <HeatmapBlock key={stock.symbol} stock={stock} />
                        ))}
                    </div>
